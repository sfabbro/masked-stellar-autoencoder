...
class TabResnetWrapper(BaseEstimator):
    def __init__(
        ...
        max_cache_mem_percent: float = 85.0,
    ):
        """
        ...
        self.max_cache_mem_percent = max_cache_mem_percent
        self.data_cache = {}
    ...
    def _load_data(self, key):
        ...
        if self.cache_data and key in self.data_cache:
        ...
        try:
            ...
            tensors = (
                ...
                torch.Tensor(eX).to(self.device),
            )

            if self.cache_data:
                if psutil.virtual_memory().percent < self.max_cache_mem_percent:
                    self.data_cache[key] = tensors
                else:
                    print(
                        f"Memory > {self.max_cache_mem_percent}%, skipping cache for {key}"
                    )
