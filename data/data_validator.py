...
class DataValidator:
    ...
    def validate_stellar_data(
        ...
        if nan_fraction > max_nan_fraction:
            validation_report["warnings"].append(
                f"High NaN fraction: {nan_fraction:.2%}"
            )
        ...
        if "G" in feature_names:
            ...
            if len(valid_g) > 0:
                if np.any(valid_g < 5) or np.any(valid_g > 25):
                    validation_report["warnings"].append(
                        "G magnitude values outside typical range [5, 25]"
                    )
    ...
    def validate_scaling_consistency(scaler, data: np.ndarray) -> bool:
        ...
        if len(scaler.scale_) != data.shape[1]:
            raise ValueError(
                f"Scaler dimension mismatch: {len(scaler.scale_)} vs {data.shape[1]}"
            )
    ...
    def check_data_leakage(
        ...
        if len(train_val_overlap) > 0:
            raise ValueError(
                f"Data leakage: {len(train_val_overlap)} samples overlap between train and validation"
            )
        ...
        if test_ids is not None:
            ...
            if len(val_test_overlap) > 0:
                raise ValueError(
                    f"Data leakage: {len(val_test_overlap)} samples overlap between validation and test"
                )
