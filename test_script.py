...
def slow_load_data(key):
    # Wait to simulate I/O slow down, but this means we overwrite the patched cache access?
    # No, we shouldn't monkey patch _load_data directly, because _load_data handles caching now.
    pass

# We can monkey patch self.datafile accessing
class SlowFile:
    def __init__(self, f):
        self.f = f
...
with h5py.File(h5_path, 'r') as f:
    wrapper.datafile = SlowFile(f)

    start = time.time()
    wrapper.pretrain_hdf(train_keys=train_keys, num_epochs=2, mini_batch=100)
    end = time.time()
    print(f"Elapsed time with cache: {end - start:.2f} seconds")
