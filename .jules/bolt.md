...
## 2025-05-24 - Prioritize Memory Scalability in Contrastive Losses
**Learning:** In PyTorch custom loss functions (e.g., RnCLoss), vectorizing row-wise contrastive metrics using 3D tensor broadcasting creates an O(N^3) memory footprint. This causes severe Out-Of-Memory errors for larger batches.
**Action:** Prioritize memory scalability by using efficient `for` loops instead of vectorization for contrastive metrics in `RnCLoss`.
