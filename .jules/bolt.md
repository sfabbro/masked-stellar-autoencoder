...
## 2026-11-20 - Avoid `torch.max(a, b)` with computed expressions
**Learning:** In PyTorch, using `torch.max(a, b)` where `a` and `b` are computed expressions (like `torch.max((q - 1) * err, q * err)`) creates multiple large intermediate tensors before computing the element-wise maximum. Optimize memory and execution time by replacing these multiple-allocation calls with mathematically equivalent expressions (e.g., `err * q - err.clamp_max(0.0)`).
**Action:** Replace `torch.max((q - 1) * err, q * err)` with `err * q - err.clamp_max(0.0)` in custom loss loops like `quantile_loss` to minimize memory allocation and execution time.
