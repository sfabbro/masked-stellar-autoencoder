...
## 2025-05-19 - PyTorch Dynamic Masking Shape Overheads
**Learning:** In PyTorch, using dynamic-shape boolean indexing (like `input[mask]`) where the resulting tensor shape varies dynamically, triggers extremely costly Device-to-Host (GPU to CPU) synchronization. The CPU must halt and wait for the GPU to finish the masking operation to determine the new size of the tensor before continuing memory allocations. Additionally, the operation involves creating new tensors. This creates severe performance bottlenecks, especially in custom loss functions evaluated many times per epoch.
**Action:** Replace `input[mask]` operations in loss functions with `input.masked_fill(~mask, 0.0)`. This operates in-place or on full-shape tensors, preserving the static shape. Follow up with a masked reduction like `.sum() / mask.sum().clamp_min(1)`. This avoids dynamic shape changes, prevents CPU-GPU syncing, and drastically speeds up the execution time. Keep standard boolean indexing for cases where the loss is unreduced (`reduction='none'`) and requires a flat 1D array.
