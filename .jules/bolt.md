...
## 2026-10-30 - Replace multiple tensor allocations in torch.max with equivalent math
**Learning:** In PyTorch, using `torch.max(a, b)` where `a` and `b` are computed expressions (like `torch.max((q - 1) * err, q * err)` in quantile loss) creates multiple large intermediate tensors before computing the element-wise maximum.
**Action:** Optimize memory and execution time by replacing these multiple-allocation calls with mathematically equivalent expressions. For quantile loss, replacing it with `err * q - err.clamp_max(0.0)` yielded a ~1.5x - 3.2x speedup by reducing intermediate allocations.
