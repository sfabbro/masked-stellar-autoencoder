...
## 2026-10-30 - Avoid torch.max on computed expressions
**Learning:** In PyTorch, using `torch.max(a, b)` where `a` and `b` are computed expressions (like `torch.max((q - 1) * err, q * err)`) creates multiple large intermediate tensors before computing the element-wise maximum. This wastes memory and causes multiple memory allocations.
**Action:** Optimize memory and execution time by replacing these multiple-allocation calls with mathematically equivalent expressions. For instance, replace `torch.max((q - 1) * err, q * err)` with `err * q - err.clamp_max(0.0)`, which eliminates the need to allocate intermediate tensors for both arguments of `torch.max`.
