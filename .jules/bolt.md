...
## 2026-10-30 - Avoid intermediate tensor allocations in torch.max
**Learning:** In PyTorch, using `torch.max(a, b)` where `a` and `b` are computed expressions (like `torch.max((q - 1) * err, q * err)`) creates multiple large intermediate tensors before computing the element-wise maximum, leading to unnecessary memory allocations and slower execution.
**Action:** Optimize memory and execution time by replacing these multiple-allocation calls with mathematically equivalent expressions. For instance, replace `torch.max((quantiles - 1) * error, quantiles * error)` with `error * quantiles - error.clamp_max(0.0)`.
