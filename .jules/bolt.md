...
## 2026-10-30 - Avoid `torch.max` on computed expressions to prevent intermediate tensor allocations
**Learning:** In PyTorch, using `torch.max(a, b)` where `a` and `b` are computed expressions (like `torch.max((q - 1) * err, q * err)`) creates multiple large intermediate tensors before computing the element-wise maximum. We benchmarked this and found it to be over 2x slower for large batches compared to mathematically equivalent expressions that avoid the max operator.
**Action:** Optimize memory and execution time by replacing these multiple-allocation calls with mathematically equivalent expressions. For example, in quantile loss, replace `torch.max((q - 1) * err, q * err)` with `err * q - err.clamp_max(0.0)`.
