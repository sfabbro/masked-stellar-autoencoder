...
## 2026-10-30 - Replace `torch.max` of computed expressions with mathematical equivalents
**Learning:** Using `torch.max(a, b)` where `a` and `b` are computed expressions (like `torch.max((q - 1) * err, q * err)`) creates multiple large intermediate tensors before computing the element-wise maximum. This wastes memory and increases execution time.
**Action:** Optimize memory and execution time by replacing these multiple-allocation calls with mathematically equivalent expressions (e.g., `err * q - err.clamp_max(0.0)`) that avoid allocating intermediate tensors and use fast built-in functions.
