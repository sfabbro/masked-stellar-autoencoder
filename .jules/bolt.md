...
## 2026-10-30 - Avoid intermediate tensor allocation in min/max expressions
**Learning:** In PyTorch, using `torch.max(a, b)` where `a` and `b` are computed expressions (like `torch.max((q - 1) * err, q * err)`) creates multiple large intermediate tensors before computing the element-wise maximum.
**Action:** Optimize memory and execution time by replacing these multiple-allocation calls with mathematically equivalent expressions (e.g., `err * q - err.clamp_max(0.0)`) that reduce the number of intermediate tensors allocated.
