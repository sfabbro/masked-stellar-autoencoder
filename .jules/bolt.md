...
## 2026-07-23 - Avoid intermediate tensor allocation via PyTorch automatic broadcasting
**Learning:** In PyTorch, explicitly expanding tensors to match the shape of a target tensor using `.expand_as(target)` before applying masking via `.masked_fill()` creates full-shape multi-dimensional intermediate tensors. This explicitly allocates unnecessary memory, especially when the resulting masked tensor is subsequently used in element-wise multiplications, bypassing PyTorch's efficient native broadcasting.
**Action:** Instead of calling `.expand_as()`, rely on PyTorch's native automatic broadcasting. Apply `.masked_fill()` directly on the appropriately shaped smaller tensor, and allow the subsequent element-wise operation (like multiplication) to handle the broadcasting implicitly. This avoids allocating full-size intermediate tensors and provides a measurable speedup.
