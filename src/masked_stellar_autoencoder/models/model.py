...
class MaskedGaussianNLLLoss(nn.Module):
    ...
    def forward(self, pred_mean, target, pred_var, target_var):
        ...
        safe_target_var = target_var.masked_fill(
            ~mask, 1.0
        )  # avoid log(0) if clamp is bypassed
        ...
        err = var + obs_var
        diff_squared = (pred_mean - safe_target).masked_fill_(~mask, 0.0) ** 2
...
class WeightedMaskedMSELoss(nn.Module):
    ...
    def forward(self, target, input, weight):
        ...
        err = (input - safe_target).masked_fill_(~mask, 0.0)
        weighted_err = (err**2) * safe_weight

        if self.reduction == "mean":
            return weighted_err.sum() / (safe_weight.sum() + self.eps)
        elif self.reduction == "sum":
...
class MaskedMSELoss(nn.Module):
    ...
    def forward(self, target, input):
        ...
        if self.reduction == "mean":
            return squared_err.sum() / mask.sum().clamp_min(1)
        elif self.reduction == "sum":
            return squared_err.sum()
...
class MaskedMAELoss(nn.Module):
    ...
    def forward(self, target, input):
        ...
        safe_target = target.masked_fill(~mask, 0.0)
        err = torch.abs(input - safe_target).masked_fill_(~mask, 0.0)
