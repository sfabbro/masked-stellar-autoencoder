...
def test_resblock_unsupported_activation():
    with pytest.raises(ValueError, match="Unsupported activation type: foo. Use 'elu', 'gelu', or 'relu'"):
...
def test_resblock_unsupported_norm():
    with pytest.raises(ValueError, match="Unsupported norm type: foo. Use 'batch' or 'layer'"):
